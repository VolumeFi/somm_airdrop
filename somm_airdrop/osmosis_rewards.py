...
class OsmosisSnapshotRewards:
    ...
    json_save_path = Path(
        '../token_rewards/old_rewards_in_units_somm/osmosis_pool_rewards.json'
    ).resolve()
    usomm_json_save_path = Path(
        '../token_rewards/osmosis_pool_rewards_usomm.json')

    def __init__(self):
        self.osmosis_data: OsmosisData = self.load_snapshot_data()
        self.wallet_to_account: Dict[Wallet,
                                     Account] = (self.osmosis_data["accounts"])
    ...
    def get_pool_to_liquidity_map(
            self, wallet_to_account: Dict[Wallet,
                                          Account]) -> Dict[PoolName, int]:
    ...
    def num_pools(self) -> int:
        pool_totals = [
            int(v['pool_total']) for v in self.pool_to_liquidity_map.values()
        ]
    ...
    def map_wallets_to_pool_share_maps(self) -> Dict[Wallet, PoolShareMap]:
        ...
        for wallet_address, wallet_row in self.wallet_to_account.items():
            ...
            for pool_amount in wallet_row['balance']:
                ...
                pool_share: float = (wallet_tokens_in_pool /
    ...
    def compute_wallet_reward_map(self) -> Dict[Wallet, float]:
        ...
        for wallet_address, pool_shares in self.wallet_to_pool_share_map.items(
            ...
            for pool_name, wallet_pool_share in pool_shares.items():
                wallet_reward_map[wallet_address] += (wallet_pool_share *
                                                      REWARD_PER_POOL)
    ...
    def compute_redistribution_amounts(self):
        ...
        plot_utils.plot_reward_distribution(
            ...
            save_path=Path(
                "../plots/osmosis_pool_rewards_usomm.png").resolve(),
            title="Osmosis LP SOMM Rewards")

    def convert_to_usomm(self):
        usomm_wallet_reward_map = {}
        for k, v in self.wallet_reward_map.items():
            usomm_wallet_reward_map[k] = int(v * USOMM_FACTOR)
        self.usomm_wallet_reward_map = usomm_wallet_reward_map

        self.usomm_json_save_path.parent.mkdir(exist_ok=True, parents=True)

        with open(self.usomm_json_save_path, 'w') as fp:
            json.dump(self.usomm_wallet_reward_map, fp)
...
if __name__ == "__main__":
    ...
    with open(
        ...
        print(f"Total airdrop rewards on Osmosis in SOMM: {total_rewards}")

    with open(
            Path('../token_rewards/osmosis_pool_rewards_usomm.json').resolve()
    ) as f:
        usomm_wallet_reward_map: Dict[Wallet, float] = json.load(f)
        total_rewards: float = sum(usomm_wallet_reward_map.values())
        print(f"Total airdrop rewards on Osmosis in uSOMM: {total_rewards}")